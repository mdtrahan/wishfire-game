...
## Completed Beads
- ORKA-bm41 (idle combat no longer crashes on forced enemy loadouts; session respawns now reuse stored forced enemy names safely)
...
- ORKA-3nlw (`in_progress`): dirty worktree cleanup and commit plan
- ORKA-tvn5 (`in_progress`): fix Djinn/Marid line-clears invalidating board playability

## Next Tasks
- Keep using live `bd` for queue decisions; treat `.beads/` as a cache only.
- Continue the remaining live `in_progress` lanes with bounded validation and explicit closeout evidence.

## Known Issues
- Shell PATH still does not include `bd` by default; automation should continue exporting `PATH="$HOME/.local/bin:$PATH"` before `bd` commands.
...
## Next Tasks
- Rewrite ORKA-6opp before assigning it again; current description is not implementation-ready.
- Reduce unresolved spec debt before claiming additional READY beads.

## Known Issues
- Unresolved issue count remains above the PM/dev guard threshold, so new bead assignment should pause until missing-spec items are clarified.
- Shell PATH still does not include `bd` by default; automation should continue exporting `PATH="$HOME/.local/bin:$PATH"` before `bd` commands.
