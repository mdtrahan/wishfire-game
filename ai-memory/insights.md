...
- Enemy death fades must be armed at the first `hp <= 0` transition, not deferred until later pending-death or respawn helpers. If fade ownership starts in `KillEnemyAt(...)` instead of damage application, grouped/AOE kills can create a visual gap where the enemy disappears and only later reappears to fade out.
- For enemy-death regressions, validate the live event order in combat with explicit debug logs before changing timings: `hp_zero -> fade_queued -> fade_complete`. If any death starts with `fade_queued` after a visible gap, the bug is lifecycle ownership, not easing or duration.
- Power Amp crit presentation must follow amp provenance, not just base crit flags. Any queued or multi-hit attack that spends an active Power Amp state must carry `powerAmpMultiplier` through to the floating-text seam, or real x2/x3 attacks will silently lose `!!`.
- When a bead narrows presentation semantics, remove inference from string decoration. If heal/damage color depends on prefixes like `+` or `-`, sign-removal beads will regress color lanes. Pass explicit `kind`/ownership metadata into the renderer instead.
- Keep attack naming and condition naming separate. For Kojonn, `Faze` is the attack presentation and `blight` is the applied condition. Logs, tooltips, and UI copy should preserve that distinction instead of collapsing the attack name into the status effect name.
- For staged FX like coin collection, a visible “snap” is often not a framerate problem but a path-ownership problem. If the next phase starts from a midpoint or target vector computed from the original spawn position instead of the current animated position, the object will appear to jump even inside one timeline. Capture live `x/y` at transition start and build the pull-in curve from there.
- If a party-wide over-time effect needs actor-side presentation, derive the render gate from the owning effect queue (`PartyRegens`) instead of only from the bar/HUD feedback seam. Otherwise the HP bar can correctly show HoT while hero sprites stay visually silent.
- For masked persistent overlays, reuse the same sprite-owned overlay helper and only add a new tone/palette lane. Creating a second overlay system for the same actor class makes regen/blight parity drift much more likely.
- Combat fail/exit gates must derive defeat from live actor entities, not only cached aggregate HUD totals like `PartyHP`. If aggregate health drifts stale while heroes still have live HP, the layout layer can falsely exit combat and then appear to “randomly” reset back through story/town/combat transitions.
- In mirrored combat rule files, any party-alive gate used for roster construction or turn-start eligibility should read live hero entities first. Using stale aggregate `PartyHP` in `getHeroes()` or turn scheduling can create split-brain combat where the layout thinks the party is dead before the actor layer does.
- Canvas-state GSAP targets must only animate plain numeric state. DOM-only properties like `transformOrigin` are invalid on HP bar state objects and surface as misleading plugin warnings during real combat updates even though the bar motion itself still runs.
- When a positive status tint reads like poison/sickness, remove the full-sprite color wash instead of retuning it endlessly. For hero HoT, a lightweight motion cue like clipped shimmer lines preserves “active healing” semantics without borrowing the debuff visual language.
- For mirrored status-effect presentation, keep the base semantic anchor and vary only the accent particles. Kojonn HoT and Faze blight both read well as “persistent masked status + slow upward particles,” but the heal lane should use bright lines/diamonds while blight uses darker purple dots so the player reads symmetry without confusion.
...
- 2026-03-21: After protected-branch conflict resolution on mirrored hot files, verify both `Scripts/` and `web-runner/modules/` for duplicated accepted lines around conditional guards before trusting the browser. A malformed duplicate `if` can surface later as a misleading `Unexpected token 'export'` syntax error at the next export boundary rather than at the actual damaged line.

## 2026-03-21 — Module-Scope Render Helpers Must Not Reach Into Frame-Local Projectors
- If a DOM/render helper is moved to module scope but still depends on a projector like `worldToCanvas` that lives inside `drawFrame()`/`main()`, pass that projector through the call seam explicitly. Do not let module-scope helpers reach for frame-local closures that are out of scope at runtime.
- Diagnostic order for browser-only render regressions after presentation refactors: check for `ReferenceError` first, then compare helper scope against the owning render loop, then thread the missing projector/state in as an explicit argument instead of duplicating coordinate math in two places.

## 2026-03-21 — Crit Presentation Must Follow Combat Provenance, Not Magnitude Thresholds
- If the UI needs crit emphasis, do not infer crits from output size like `damage >= X`. Balance scale changes, DoT ticks, and heal values will all make that heuristic lie. Carry the actual combat verdict (`didCrit` / `isCrit`) from the owner seam into the floating-text payload.
- Diagnostic order for missing crit emphasis: check where crit is decided, check whether that verdict survives delayed hit queues or heal roll helpers, then verify `SpawnDamageText`/render payloads carry explicit `isCrit` instead of reconstructing crit from the final amount.
- Crit visuals must stay white unless a bead explicitly changes palette. If a new DOM animation replaces legacy canvas text, preserve the previous text color first and layer glow/flash effects on top; otherwise presentation regressions slip in under “animation improvements.”
- Crit provenance must survive deferred application seams. If a queued hit, DoT/HoT tick, or amp-modified follow-up is applied later from `app.js`, pass explicit `isCrit` metadata back into `ApplyDamageToTarget`/`SpawnDamageText` instead of trying to infer it from magnitude at render time.
- When a bead requests a narrow presentation change, preserve adjacent visual semantics exactly. For combat text, damage/crit color, heal color, and motion are separate contracts; changing one without explicit scope for the others is a regression even if the new look is internally consistent.
- For crit-text presentation specifically, treat `!!` as the emphasis unless the bead explicitly authorizes more. Do not add glow, flash, palette swaps, or other secondary callouts under the same bead just because the animation system makes them easy.
- For actor death visuals, snapshot a short-lived render ghost before removing the gameplay entity. If the entity is deleted first and the effect is tied to slot state or respawn timing, the player sees a pop-out instead of a fade and the visual drifts from the actor owner seam.
- When a UI animation spec assumes DOM but the runtime owner seam is canvas, preserve the behavior contract rather than the implementation detail. Use GSAP-driven numeric presentation state behind the canvas draw loop instead of forcing a second rendering system just to match sample markup literally.
- If a status effect already has an actor-owned runtime ledger, derive persistent overlays from that ledger instead of slot tint or one-shot flash state. For Faze blight, `EnemyDamageOverTime[targetUID]` is the owner seam; drawing the purple overlay from that actor-bound effect state keeps the visual attached through movement and clears it naturally on expiry/death.
- For presentation beads, preserve the established lane semantics before styling the new effect. In this game, hero-heal feedback belongs on the actor seam, not the HP bar seam, and the plus/burst palette should match the existing heal text color instead of introducing a new glow-heavy color treatment.
- If the combat model treats hero healing as party-wide, actor-seam visuals must fan out to the whole party even when the numeric heal text is summarized at the bar seam. Do not key hero-only effects solely off `targetKind === 'hero'` if the underlying heal event is still one party-wide bar update.
- For death fades, do not remove the live actor and then render a separate ghost copy. That handoff creates pop/ghost artifacts, especially on AOE multi-kills. Keep the original enemy entity alive in a non-interactable dying state, fade that single actor once, and only remove it after the fade completes.
- If a new actor-seam heal effect already communicates HoT clearly, remove duplicate bar-lane text for the same HoT tick. Replace the noise with a short positive bar overlay instead of stacking both signals.
- Module-scope presentation helpers must not capture frame-local projectors. If a new animation needs `worldToCanvas`, thread the projector in from `drawFrame()` or defer the conversion to the render seam instead of letting a helper reach for an out-of-scope render local.
- When a status effect already has a strong persistent visual like Faze blight’s purple overlay, suppress redundant floating text on each enemy tick if it harms readability. Keep the damage and status lifetime intact, but let the owned overlay carry the communication instead of stacking noisy number spam on top.
- For enemy death fades, slot arrays are not the source of truth once the actor enters a dying state. Respawn and spawn eligibility must check live entity occupancy for that slot, including actors with `deathFadeUntil > now`, or a replacement can legally spawn into a slot that is still visually occupied and create random-looking ghost/reappear artifacts.
- When death ghosting looks random, check for duplicate render ownership before tuning timings again. If two enemy entities can coexist for one slot, the render loop must collapse them to one visible owner per slot, with the dying actor taking priority until its fade completes.
- Separate masked status ownership from ambient particle ownership. If a status effect uses a clipped sprite wash to mark the affected actor, that does not mean its accent particles should also clip. Default rule: keep the base overlay actor-bound and masked, but let shimmer dots/lines/diamonds float above the actor unless the bead explicitly asks for silhouette-only particles.
- For negative-status particles, readability comes more from edge contrast than raw glow. If purple DoT dots disappear into enemy art, darken the fill slightly and add a darker outline before increasing size or count; that preserves subtlety while making the particles identifiable at gameplay speed.
- Initiative hero turns must not use broad “restore pickability” fallbacks. If the app runtime ever force-sets `CanPickGems = true` for hero turns, guard it with the full idle window: hero type, phase 0, no refill, not busy, no pending skill, no action in progress, and no deferred advance. Otherwise initiative/deferred turns can leak back into player control and look like infinite hero turns.
- Shared-party healing must preserve the exact requested total when syncing back onto individual heroes. If `PartyHP` is redistributed by flooring each hero’s proportional share independently, small HoT ticks can move the bar while no hero actually gains HP. Use remainder distribution after flooring so at least one hero receives the extra point and total live hero HP matches shared `PartyHP`.
- For forced monochrome QA boards, do not “fix” turn leaks by turning forced-color refill off. Keep the forced-color policy intact and trace the post-match lifecycle instead: first check whether support colors enter action phase, then check whether deferred advance is blocked by transient refill/merge state, then check whether `BoardFillActive` can get stranded after a no-work refill pass. A stale refill flag can look like a turn bug because it freezes support-color handoff even when the board is already full.
