...
export function getIdleAutoplayColorPriorityTiers({ heroName = '', partyHpRatio = null } = {}) {
  ...
  if (ratio != null && ratio < 0.6) {
    pushTier(tiers, [COLOR_HEAL]);
  }

  if (heroColor != null) {
    pushTier(tiers, [heroColor]);
  }

  pushTier(tiers, [COLOR_PURPLE]);
  pushTier(tiers, otherNonHealColors);
  pushTier(tiers, [COLOR_HEAL]);
