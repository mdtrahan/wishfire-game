...
export function isAbleToActSlot(actor = {}) {
  ...
  const normalizeStatus = value => {
    ...
    return `|${[value.statusEffect, value.status, value.type, value.name, value.key,
      ...Object.entries(value).filter(([, entry]) => entry === true).map(([key]) => key)]
      .map(entry => String(entry || '').toLowerCase()).join('|')}|`;
  };
  const statusValues = [
    actor.status,
    actor.state,
    ...(Array.isArray(actor.statuses) ? actor.statuses : [actor.statuses]),
    ...(Array.isArray(actor.statusEffects) ? actor.statusEffects : [actor.statusEffects]),
  ].map(normalizeStatus);
