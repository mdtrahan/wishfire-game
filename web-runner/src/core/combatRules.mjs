...
const debuffs=new Set(['atkDown','defDown','magDown','resDown','spdDown','blind','silence','dot','taunted','delayNextTurn']);
...
export function turnStart(ctx,actor,serial){
 ...
 for(const owned of ctx.actors||[])if(owned!==actor)owned.statuses=statuses(owned).filter(s=>!(s.expiresOnSourceTurn&&s.sourceUID===actor.uid));
...
function damageTarget(ctx,source,target,amount,origin){
 ...
 if(actual>0)ctx.onDamage?.(source,target,actual,{...origin,targetWasLiving:before>0});
 if(target.hp<=0){resolveKO(target);ctx.onKO?.(target);}
 return actual;
}
function accuracy(ctx,source,target,skill,origin){
 const evade=statuses(target).find(s=>s.statusEffect==='evadeNext'&&s.magnitude>0);
 if(evade){evade.magnitude=0;evade.duration=0;target.statuses=statuses(target).filter(s=>s!==evade);return false;}
...
export function resolveSkill(ctx,source,skill,targetIds,origin={}){
 ...
 if(source.kind==='enemy'&&skill.targetType==='enemy'&&!skill.ignoresProvoke){const taunts=statuses(source).filter(s=>s.statusEffect==='taunted'&&s.duration>0).map(s=>({a:ctx.actors.find(a=>a.uid===s.sourceUID&&a.hp>0),s})).filter(entry=>entry.a).sort((a,b)=>b.s.appliedOrder-a.s.appliedOrder);if(taunts.length)targets=[taunts[0].a];else{const provocations=ctx.actors.filter(a=>a.kind!==source.kind&&a.hp>0).flatMap(a=>statuses(a).filter(s=>s.statusEffect==='provoke').map(s=>({a,s}))).sort((a,b)=>b.s.appliedOrder-a.s.appliedOrder);if(provocations.length)targets=[provocations[0].a];}}
   ...
   if(effect.effectType==='damage'){
    for(let hit=0;hit<(effect.hits||1);hit++){
     ...
     if(!coverChosen){coverChosen=true;if(skill.targetType==='enemy'){
      ...
      if(covers.length){defender=covers[0].a;const coverStatus=covers[0].s;if(coverStatus.coverOnce)original.statuses=statuses(original).filter(s=>s!==coverStatus);const armedCounter=statuses(defender).find(s=>s.statusEffect==='counter'&&s.counterArmedByCover);if(armedCounter)armedCounter.counterTargetUID=source.uid;if(coverStatus.coverCanEvade&&!accuracy(ctx,source,defender,{...skill,accuracy:1,ignoresBlind:true},meta))continue;}
     }}
     const conditional = effect.ifTargetStatus && hasStatus(defender, effect.ifTargetStatus) ? Number(effect.conditionalMultiplier || 1) : 1;
     const amount=(((defender===original&&(!effect.fixedTargetUID||defender.uid===effect.fixedTargetUID)?effect.fixedDamage:undefined)??ctx.calculateDamage(source,defender,skill.tags?.includes('magic')?'magic':'melee'))*(effect.potency??1)*conditional);
     damageTarget(ctx,source,defender,amount,meta);defenders.set(defender.uid,defender);
     if(hasStatus(defender,'mark')) defender.statuses=statuses(defender).filter(s=>s.statusEffect!=='mark');
    }
   }else if(effect.effectType==='status'&&recipient.hp>0){const refreshed=applyStatus(ctx,source,recipient,effect,meta);ctx.onStatus?.(source,recipient,effect,{refreshed});}
   else if(['heal','revive'].includes(effect.effectType)){
    const before=recipient.hp;if(effect.effectType==='revive'?before===0:before>0){recipient.hp=clamp(before+(effect.amount??Math.floor(recipient.maxHP*(effect.potency??0.25))),0,recipient.maxHP);recipient.isAlive=recipient.hp>0;const delta=Math.max(0,recipient.hp-before);if(delta>0)ctx.onHeal?.(source,recipient,delta,effect);
}
 ...
 if(!meta.isCounter&&!ctx.isOver?.()){
  ...
  for(const actor of counters){if(ctx.isOver?.())break;if(actor.hp<=0||source.hp<=0||!hasStatus(actor,'counter'))continue;
   const status=statuses(actor).find(s=>s.statusEffect==='counter');if(status.counterArmedByCover&&Number(status.counterTargetUID||0)!==Number(source.uid||0))continue;resolveSkill(ctx,actor,{skillId:'counter',targetType:'enemy',tags:['physical'],effects:[{effectType:'damage',potency:status.magnitude||T.counterPotency}]},[source.uid],{isCounter:true,flowOwnerUID:status.flowOwnerUID??meta.flowOwnerUID,excludedFlowUIDs:[...(meta.excludedFlowUIDs||[]),...(status.excludedFlowUIDs||[])]});if(status.counterArmedByCover)actor.statuses=statuses(actor).filter(s=>s!==status);
  }
 }
 return true;
}
