...
export function createSimulationCoreEnemySkillChoiceResolution({
  ...
  const normalized = {
    ...
    criticalAlliesCount: Math.max(0, Math.trunc(Number(criticalAlliesCount || 0))),
    boardReady: Number(boardReady || 0) === 1 ? 1 : 0,
    behaviorTurn: Math.max(0, Math.trunc(Number(behaviorTurn || 0))),
    lastBehaviorSkillCode: Math.trunc(Number(lastBehaviorSkillCode ?? -1)),
    roll: Number(roll || 0),
  ...
  const selectedCode = Number(exports.enemy_skill_choice_selected_code_shadow(
    ...
    normalized.criticalAlliesCount,
    normalized.boardReady,
    normalized.behaviorTurn,
    normalized.lastBehaviorSkillCode,
    normalized.roll,
    normalized.healRoll,
  ));
