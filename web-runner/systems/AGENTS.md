...
- Impact presentation uses the pale diffused glance ring for non-critical damage at or below 5% target max HP. Center that ring vertically on the target's standing Y while retaining the attacker-facing contact X. Stronger Runa and Kaja basics use blue and purple impacts at 1.6x the shared regular size; stronger physical basics use the red-orange melee flare. Center regular contact on each raster's painted alpha bounds, then place it on the torso side facing the attacker.
