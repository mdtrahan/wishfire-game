...
- Quest-QA scenario controls enter the production Combat layout, start from one shared fresh-session reset, and remain behind the fixture turn hold plus dev-tooling pause until explicit QA resume. Presentation frames continue under that scheduler hold so live combat effects remain inspectable. Closing diagnostics preserves the scenario pause. Opening choices and AF specials keep their production selection callbacks.
