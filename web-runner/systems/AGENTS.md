...
- heroCommandUI.mjs owns decorative hero status cards only. Cards show the cropped portrait, HP, SP and personal FLOW, with a steady cyan highlight for the scheduled living actor and a gold-ready FLOW state. Clicking that active card reopens the current Hero Turn Card Fan draw. The retired command editor, native skill buttons, prepared queue controls, Auto, Reload and Menu entrypoints are absent from player presentation. Enemy and ally taps remain owned by the existing battlefield selectors while a targeted fan card is pending. The underlying heroCommands native resolver remains available to the fan and dev autoplay seams.
