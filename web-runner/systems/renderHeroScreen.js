...
export function renderHeroScreen({canvas,gameState,fnContext,heroPortraitImages,onClose}) {
 ...
 if(key!==lastKey){lastKey=key;content.replaceChildren();
  ...
  const info=el('div',null,identity);el('h1',d.name,info);el('p',`${d.role} · ${d.flowMode} FLOW`,info);el('p',`Orb passive: ${FLOW_MODES[d.flowMode]}. Orbs grant 10 FLOW to a random living hero.`,info);el('p',`${hero.actionSlotsPerTurn??d.actionSlotsPerTurn} actions per turn`,info);el('p',`Level ${hero.currentLevel} / ${hero.maxLevel}`,info);
