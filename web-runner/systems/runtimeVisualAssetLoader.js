...
export async function loadRuntimeVisualAssets({
  ...
  const loadCoreVisuals = async () => {
    ...
    tasks.push(
      ...
      (async () => { images.CombatHealMotes = await loadImage(assetUrl('images/vfx_heal_motes.png')); })(),
      (async () => { images.CombatGroupHealRain = await loadImage(assetUrl('images/vfx_group_heal_rain.png')); })(),
