...
export function AdvanceTurn(ctx) {
  ...
  if (dynamicInitiativeAuthorityApplied) {
    const authorityCurrent = getDynamicInitiativeAuthorityCurrent(g);
    if (authorityCurrent) {
      const authorityIndex = afterQueue.findIndex(slot => Number(slot.uid || 0) === Number(authorityCurrent.uid || 0));
      if (authorityIndex !== -1) afterIndex = authorityIndex;
      afterSlot = { ...authorityCurrent, spd: Number(GetEffectiveStat(ctx, GetActorByUID(ctx, authorityCurrent.uid), 'SPD') || 0) };
    }
  }
  if (!dynamicInitiativeAuthorityApplied && shouldAutoCorrectImproperRepeat({
    timeMode,
  ...
  if (beforeSlot && afterSlot && Number(beforeSlot.uid || 0) === Number(afterSlot.uid || 0)) { if (dynamicInitiativeAuthorityApplied) repeatSource = 'dynamic_progress_math'; else if (audit.lastQueueMutation?.source === 'explicit_mechanic') repeatSource = 'explicit_mechanic'; else if (audit.lastQueueMutation?.source === 'collapse_after_future_slot_removal') repeatSource = 'collapse_after_future_slot_removal'; else if (rolloverCandidate) repeatSource = 'cycle_rollover'; else if (timeMode) repeatSource = 'non_compliant_scheduler_behavior'; }
  recordTurnSchedulerEvent(ctx, 'pointer_advance', { cause: dynamicInitiativeAuthorityApplied ? 'dynamic_initiative_authority' : (timeMode ? 'time_select_next' : (rolloverCandidate ? 'round_cycle_rollover' : 'round_pointer_increment')), beforeUID: beforeSlot ? beforeSlot.uid : 0, afterUID: afterSlot ? afterSlot.uid : 0, beforeIndex, afterIndex, repeatSource, rolloverCandidate, queue: afterQueue });
}
