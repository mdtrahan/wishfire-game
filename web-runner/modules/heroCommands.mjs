...
export function canUseHeroCommand(ctx,actorUID){const g=ctx.state.globals,hero=ctx.state.entities.find(a=>a.uid===actorUID&&a.kind==='hero'),roster=ctx.callFunction('GetEnemyRosterStability'),barrier=derivePresentationTurnBarrier({globals:g}),checks={hero:!!hero&&hero.hp>0,nativeBattleOpen:!g.NativeBattleEnded,runtime:g.GamePhase==='RUNTIME',battleStart:!g.BattleStartActive,idle:!g.IsPlayerBusy,turnPhase:Number(g.TurnPhase)===0,currentTurn:Number(ctx.callFunction('GetCurrentTurn'))===actorUID,roster:roster?.stable===true,presentation:barrier.canClaimCombatAction};const allowed=Object.values(checks).every(Boolean);if(g.QaFixtureHoldTurn||g.QaFixtureExplicitAction)g.QaFixtureProcessTurnGate={...(g.QaFixtureProcessTurnGate||{}),reason:allowed?'hero-command-eligible':'hero-command-gate',commandChecks:checks,roster,barrier};return allowed;}
