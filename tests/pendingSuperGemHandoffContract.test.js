...
const modulePath = path.join('file://', __dirname, '..', 'web-runner', 'src', 'core', 'pendingSuperGemHandoff.mjs');

test('rejected pending supergem handoff clears stale supergem action and leaves refill eligible', async () => {
...
test('resolved pending supergem handoff preserves existing busy handoff behavior', async () => {
  const mod = await import(modulePath);
  const globals = {
    PendingSkillID: 'HERO_SINGLE',
    PendingActor: 101,
...
test('normal pending skill handoff still executes skill and clears target state', async () => {
  const mod = await import(modulePath);
  const globals = {
    PendingSkillID: 'HERO_SINGLE',
    PendingActor: 101,
