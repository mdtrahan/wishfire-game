...
test('Grow is a mirrored tiered party draw card with cap and roll metadata', () => {
  for (const modulePath of [runtimePath, scriptsPath]) {
    ...
    assert.equal(partyGrow.effect.acceptanceChancePct, 32);
    assert.deepEqual(plain(partyGrow.effect.powerAmpPctByTier), [20, 30, 50]);
    assert.deepEqual(plain(partyGrow.effect.maxHpPenaltyPctByTier), [15, 20, 25]);
    assert.equal(partyGrow.effect.defensePenaltyPctByTier, undefined);
  }
});

test('Grow selection advances tier, rolls living non-grown heroes, and applies persistent Max HP tradeoff state', () => {
  for (const modulePath of [runtimePath, scriptsPath]) {
    const mod = loadModule(modulePath);
    const ctx = makeContext();

    const { selected } = selectGrow(mod, ctx, [0, 0, 0.31, 0.33, 0.1]);
    ...
    assert.deepEqual(plain(Object.keys(growState.heroes).sort()), ['100', '102']);
    assert.equal(growState.heroes['100'].powerAmpPct, 20);
    assert.equal(growState.heroes['100'].maxHpPenaltyPct, 15);
    assert.equal(growState.heroes['102'].powerAmpMultiplier, 1.2);
    assert.equal(mod.GetPowerAmpMultiplierForActor(ctx, 100), 1.2);
    assert.equal(mod.GetPowerAmpMultiplierForActor(ctx, 101), 0);
    assert.equal(ctx.state.entities[0].maxHP, 85);
    assert.equal(ctx.state.entities[0].hp, 68);
    assert.equal(ctx.state.entities[2].maxHP, 85);
    assert.equal(ctx.state.entities[2].hp, 51);
    assert.equal(ctx.state.globals.PartyMaxHPByIndex[0], 85);
    assert.equal(ctx.state.globals.PartyHPByIndex[0], 68);
    assert.equal(ctx.state.globals.PartyMaxHPByIndex[2], 85);
    assert.equal(ctx.state.globals.PartyHPByIndex[2], 51);
    assert.equal(ctx.state.globals.PartyMaxHP, 370);
    assert.equal(ctx.state.globals.PartyHP, 189);
    ...
    assert.equal(ctx.state.entities[1].maxHP, 100);
    assert.equal(ctx.state.entities[1].hp, 70);

    const queue = ctx.state.globals.GrowAcquisitionQueue;
    assert.equal(queue.length, 3);
    assert.deepEqual(plain(queue.map(event => event.heroUID)), [100, 101, 102]);
    assert.deepEqual(plain(queue.map(event => event.accepted)), [1, 0, 1]);
    assert.ok(queue.every(event => event.modalClosedAtResolution === 1));
    assert.ok(queue[0].presentationAt < queue[1].presentationAt);
    assert.ok(queue[1].presentationAt < queue[2].presentationAt);
    ...
    assert.equal(ctx.state.globals.PowerAmpVisualByUID[101], undefined);
  }
});
...
test('Grow tier updates existing grown heroes without rerolling and leaves the pool after tier three', () => {
  for (const modulePath of [runtimePath, scriptsPath]) {
    ...
    assert.deepEqual(plain(Object.keys(mod.GetGrowSkillState(ctx).heroes)), ['100']);
    assert.equal(ctx.state.globals.GrowAcquisitionQueue.length, 3);

    ctx.state.globals.time += 1;
    const second = selectGrow(mod, ctx, [0, 0, 0.1, 0.1]);
    assert.equal(second.selected.skill.rank, 2);
    assert.equal(ctx.state.globals.GrowTier, 2);
    assert.deepEqual(plain(ctx.state.globals.GrowAcquisitionQueue.map(event => event.heroUID)), [101, 102]);
    ...
    assert.equal(mod.GetGrowSkillState(ctx).heroes['100'].powerAmpPct, 30);
    assert.equal(mod.GetGrowSkillState(ctx).heroes['100'].maxHpPenaltyPct, 20);
    assert.equal(ctx.state.entities[0].maxHP, 80);
    assert.equal(ctx.state.entities[0].hp, 64);
    assert.equal(ctx.state.entities[1].maxHP, 80);
    assert.equal(ctx.state.entities[1].hp, 56);
    assert.equal(ctx.state.entities[2].maxHP, 80);
    assert.equal(ctx.state.entities[2].hp, 48);
    assert.equal(mod.GetEffectiveStat(ctx, ctx.state.entities[0], 'DEF'), 100);
    ...
    assert.equal(mod.GetGrowSkillState(ctx).heroes['100'].powerAmpPct, 50);
    assert.equal(mod.GetPowerAmpMultiplierForActor(ctx, 100), 1.5);
    assert.equal(ctx.state.entities[0].maxHP, 75);
    assert.equal(ctx.state.entities[0].hp, 60);
    assert.equal(ctx.state.entities[1].maxHP, 75);
    assert.equal(ctx.state.entities[1].hp, 52);
    assert.equal(ctx.state.entities[2].maxHP, 75);
    assert.equal(ctx.state.entities[2].hp, 45);
    assert.equal(ctx.state.globals.PartyMaxHPByIndex[0], 75);
...
test('Grow tier updates remain percentage-based when natural Max HP increases', () => {
  for (const modulePath of [runtimePath, scriptsPath]) {
    ...
    assert.equal(ctx.state.entities[0].maxHP, 96);
    assert.equal(ctx.state.entities[0].hp, 76);
    assert.equal(mod.GetGrowSkillState(ctx).heroes['100'].baseMaxHP, 120);
    assert.equal(ctx.state.globals.PartyMaxHPByIndex[0], 96);
    assert.equal(ctx.state.globals.PartyHPByIndex[0], 76);
  }
});
