...
test('normal party skill draught excludes Chain Pop from random and forced card draws', () => {
  for (const modulePath of [runtimePath, scriptsPath]) {
    const mod = loadModule(modulePath);
    const { ctx } = makeContext();
    installSequenceRandom(ctx, [0.7, 0, 0]);
    ...
    assert.equal(opened.candidates.length, 3);
    assert.equal(
      opened.candidates.some(candidate => candidate.id === 'party_chain_pop'),
      false,
      'Chain Pop should not be reachable through normal party skill draw',
    );

    const { ctx: forcedCtx } = makeContext();
    installSequenceRandom(forcedCtx, [0, 0, 0]);

    const forcedOpened = mod.ForceAstralFlowSkillDraught(forcedCtx, 100, 'party_chain_pop');

    assert.equal(forcedOpened.ok, true);
    assert.equal(forcedOpened.candidates.length, 3);
    assert.equal(
      forcedOpened.candidates.some(candidate => candidate.id === 'party_chain_pop'),
      false,
      'Chain Pop should not be reachable through forced party skill draw',
    );
  }
});
