...
test('normal party skill draught excludes Momentum from random and forced card draws', () => {
  for (const modulePath of [runtimePath, scriptsPath]) {
    const mod = loadModule(modulePath);
    const { ctx } = makeContext();
    installSequenceRandom(ctx, [0.2, 0, 0]);
    ...
    assert.equal(opened.candidates.length, 3);
    assert.equal(
      opened.candidates.some(candidate => candidate.id === 'party_momentum'),
      false,
      'Momentum should not be reachable through normal party skill draw',
    );

    const { ctx: forcedCtx } = makeContext();
    installSequenceRandom(forcedCtx, [0, 0, 0]);

    const forcedOpened = mod.ForceAstralFlowSkillDraught(forcedCtx, 100, 'party_momentum');

    assert.equal(forcedOpened.ok, true);
    assert.equal(forcedOpened.candidates.length, 3);
    assert.equal(
      forcedOpened.candidates.some(candidate => candidate.id === 'party_momentum'),
      false,
      'Momentum should not be reachable through forced party skill draw',
    );
  }
});
