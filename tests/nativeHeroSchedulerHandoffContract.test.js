...
function loadAdvanceHarness({ livingEnemy = true, currentUID = 2, actionInProgress = 0, playerBusy = 0 } = {}) {
  ...
  const entities = [
    { uid: 3, kind: 'hero', name: 'Runa', hp: 40, stats: { SPD: 12 } },
  ...
  const globals = {
    ...
    CurrentTurnIndex: 0,
    TurnOrderArray: [{ uid: 2, type: 0, spd: 10 }, { uid: 3, type: 0, spd: 12 }, { uid: 9, type: 1, spd: 5 }],
    ActionOwnerUID: 3,
    DeferAdvance: 1,
...
test('Runa completion advances from the owner before Hondo claims the next living turn', () => {
  const harness = loadAdvanceHarness({ livingEnemy: true, currentUID: 2 });
...
test('owner reconciliation stays inert for terminal or enemy-current states', () => {
  const noEnemy = loadAdvanceHarness({ livingEnemy: false, currentUID: 2 });
  ...
  const enemyCurrent = loadAdvanceHarness({ livingEnemy: true, currentUID: 9 });
  ...
  const activeAction = loadAdvanceHarness({ livingEnemy: true, currentUID: 2, actionInProgress: 1 });
