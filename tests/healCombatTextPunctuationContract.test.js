...
for (const filePath of MIRRORS) {
  test(`heal combat text ends with exclamation punctuation in ${path.relative(process.cwd(), filePath)}`, () => {
    ...
    assert.match(src, /critically healed for \$\{appliedHeal\}!/);
    assert.match(src, /healed for \$\{appliedHeal\}!/);
    assert.match(src, /critically heals her allies!/);
    assert.match(src, /heals her allies!/);
    assert.match(src, /critically heals \$\{target\.name \|\| 'ally'\} for \$\{appliedHeal\}!/);
    assert.match(src, /heals \$\{target\.name \|\| 'ally'\} for \$\{appliedHeal\}!/);
