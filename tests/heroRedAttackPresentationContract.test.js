...
test('hero red single-target attacks use shared 4-hit clustered presentation in both runtime mirrors', () => {
  ...
  for (const src of [runtimeSrc, mirrorSrc]) {
    ...
    assert.match(src, /LogCombat\(ctx, `\$\{actorName\} hit \$\{target\.name \|\| '\?'\} for \$\{totalBurstDamage\}!`\);/);
