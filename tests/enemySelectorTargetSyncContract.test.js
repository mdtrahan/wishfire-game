...
test('pending enemy selector uses current selected enemy before queued hit fallback', () => {
  ...
  assert.match(src, /\.replace\(\s*"const resolvedSelectedUid = pendingHitTargetUID \|\| selectedUid;",\s*"const resolvedSelectedUid = selectedUid \|\| pendingHitTargetUID;",\s*\)/s);
