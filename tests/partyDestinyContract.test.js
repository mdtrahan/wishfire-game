...
test('dev panel relies on Force Draw skill id instead of a Destiny trigger button', () => {
  const runtimeSrc = fs.readFileSync(path.join(__dirname, '..', 'web-runner', 'systems', 'devToolingRuntime.js'), 'utf8');
  assert.match(runtimeSrc, /data-devtool-skill-id/);
  assert.match(runtimeSrc, /data-devtool-force-skill-draught/);
  assert.match(runtimeSrc, /ForceAstralFlowSkillDraught/);
  assert.doesNotMatch(runtimeSrc, /data-devtool-trigger-destiny/);
  assert.doesNotMatch(runtimeSrc, /triggerDestiny/);
