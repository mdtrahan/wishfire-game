...
test('dev panel exposes Destiny trigger without inlining effect logic', () => {
  ...
  const triggerDestinyBlock = appSrc.match(/devToolingDom\.triggerDestiny\.addEventListener\('click', \(\) => \{[\s\S]*?\n  \}\);\n  devToolingDom\.clearSessionSkills/);
  assert.ok(triggerDestinyBlock);
  assert.doesNotMatch(triggerDestinyBlock[0], /ApplyPartyHeal/);
});
