...
test('app adapts canvas HP bars through animated front and lag state', () => {
  ...
  assert.match(src, /if \(clamped >= 0\.4\) \{/);
  assert.match(src, /if \(clamped >= 0\.1\) \{/);
  ...
  assert.match(src, /updateAnimatedHP\(\{\s*current: state\.globals\.PartyHP \|\| 0,/s);
