...
test('Runa basic magic damage awards one Tactician AF step without an enemy-death gem',()=>{const {h,e,g,ctx}=battle('Runa');assert.equal(commands.executeHeroCommand(ctx,{actorUID:1,targetUID:2}),true);commands.resolveNativeCommandStep(ctx,g.PendingHeroHits.shift());assert.equal(e.hp,90);assert.equal(h.flow,10);assert.equal(g.FlowOrbs,undefined);});
