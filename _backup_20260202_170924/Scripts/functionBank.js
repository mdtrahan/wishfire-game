...
export function BuildTurnOrder(ctx) {
  const g = getGlobals(ctx);
  const actors = [];
  const newSpawn = g.NewSpawnUIDs || {};

  for (const h of getHeroes(ctx)) {
    if ((h.hp ?? 0) > 0) {
      const base = Number(h.spd ?? h.stats?.SPD ?? 0);
      const spdVal = base + (g.PartyBuff_SPD || 0);
      actors.push({ uid: h.uid, spd: spdVal, type: 0 });
    }
  }

  for (const e of getEnemies(ctx)) {
    if ((e.hp ?? 0) > 0) {
      const base = Number(e.spd ?? e.stats?.SPD ?? 0);
      const spdVal = GetEffectiveStat(ctx, e, 'SPD');
      actors.push({ uid: e.uid, spd: spdVal, type: 1 });
    }
  }

  const startMode = g.BattleStartMode;
  if (startMode && !g.BattleStartResolved) {
    const heroes = actors.filter(a => a.type === 0).sort((a, b) => b.spd - a.spd);
    const enemies = actors.filter(a => a.type === 1).sort((a, b) => b.spd - a.spd);
    g.TurnOrderArray = (startMode === 'ambush') ? enemies.concat(heroes) : heroes.concat(enemies);
    g.BattleStartResolved = 1;
    g.NewSpawnUIDs = {};
    if (!g.ExtraTurnGranted) g.ExtraTurnGranted = {};
    console.log('[TURN] BuildTurnOrder (start-mode):', (g.TurnOrderArray || []).map(a => {
      ...
      return `${actor ? actor.name : a.uid} ${Math.round(base + buff)}/${Math.round(base)} (+${Math.round(buff)})`;
    }).join(' | '));
    const heroStats = getHeroes(ctx).map(h => {
      const baseATK = Number(h.stats?.ATK ?? h.ATK ?? 0);
      const baseDEF = Number(h.stats?.DEF ?? h.DEF ?? 0);
      const baseMAG = Number(h.stats?.MAG ?? h.MAG ?? 0);
      const baseRES = Number(h.stats?.RES ?? h.RES ?? 0);
      const baseSPD = Number(h.stats?.SPD ?? h.SPD ?? 0);
      const curATK = baseATK + (g.PartyBuff_ATK || 0);
      const curDEF = baseDEF + (g.PartyBuff_DEF || 0);
      const curMAG = baseMAG + (g.PartyBuff_MAG || 0);
      const curRES = baseRES + (g.PartyBuff_RES || 0);
      const curSPD = baseSPD + (g.PartyBuff_SPD || 0);
      return `${h.name || h.uid} ATK ${curATK}/${baseATK} (+${g.PartyBuff_ATK || 0}) DEF ${curDEF}/${baseDEF} (+${g.PartyBuff_DEF || 0}) MAG ${curMAG}/${baseMAG} (+${g.PartyBuff_MAG || 0}) RES ${curRES}/${baseRES} (+${g.PartyBuff_RES || 0}) SPD ${curSPD}/${baseSPD} (+${g.PartyBuff_SPD || 0})`;
    }).join(' | ');
    if (heroStats.length) console.log('[STATS] Heroes:', heroStats);
    return;
  }

  actors.sort((a, b) => b.spd - a.spd);
  if (!g.InitialSpawn && newSpawn && Object.keys(newSpawn).length) {
    let spdOppMax = 0;
    for (const a of actors) {
      if (a.type === 0) spdOppMax = Math.max(spdOppMax, a.spd || 0);
    }
    const ratio = g.SpeedDoubleRatio || 2.0;
    const keep = [];
    const moved = [];
    for (const a of actors) {
      if (a.type === 1 && newSpawn[a.uid]) {
        if (spdOppMax > 0 && (a.spd || 0) >= spdOppMax * ratio) {
          keep.push(a);
        } else {
          moved.push(a);
        }
    ...
    g.TurnOrderArray = keep.concat(moved);
    g.NewSpawnUIDs = {};
  } else {
    g.TurnOrderArray = actors;
  }
  if (!g.ExtraTurnGranted) g.ExtraTurnGranted = {};
  console.log('[TURN] BuildTurnOrder:', (g.TurnOrderArray || []).map(a => {
    const actor = GetActorByUID(ctx, a.uid);
    const base = actor ? Number(actor.stats?.SPD ?? actor.SPD ?? 0) : 0;
    const buff = actor && actor.kind === 'hero' ? (g.PartyBuff_SPD || 0) : 0;
    return `${actor ? actor.name : a.uid} ${Math.round(base + buff)}/${Math.round(base)} (+${Math.round(buff)})`;
...
export function ApplyDamageToTarget(ctx, uid, dmg) {
  ...
  if (dx != null && dy != null && g.SpawnDamageText !== 0) {
    SpawnDamageText(ctx, dmg, dx, dy, 'damage');
  }
  if (t.hp === 0) {
...
export function HeroAttackSingle(ctx, heroUID, targetUID) {
  ...
  LogCombat(ctx, `${actorName} hit ${targetName} for ${dmg}!`);
}
...
export function HeroAttackAOE(ctx, heroUID) {
  ...
  const aoeName = ['Pummel', 'Swipe', 'Burst', 'Faze'][heroIndex] || 'AOE';
  let totalDamage = 0;
  const g = getGlobals(ctx);
  const isKojonn = actor && actor.name === 'Kojonn';
  const durationTurns = Math.floor(Math.random() * 6) + 3; // 3-8
  const turnTimeEquivalent = Math.max(1, g.TurnTimeEquivalent || 1);
  const totalTicks = durationTurns * turnTimeEquivalent;
  if (isKojonn) {
    if (!g.EnemyDoTs) g.EnemyDoTs = {};
    LogCombat(ctx, `${actorName} uses Faze over time!`);
  }
  for (const e of getEnemies(ctx)) {
    const dmg = CalculateDamage(ctx, heroUID, e.uid, mode);
    if (isKojonn) {
      const perTick = totalTicks > 0 ? (dmg / totalTicks) : dmg;
      if (!g.EnemyDoTs[e.uid]) g.EnemyDoTs[e.uid] = [];
      g.EnemyDoTs[e.uid].push({
        remainingTicks: totalTicks,
        dmgPerTick: perTick,
        nextTickAt: (g.time || 0) + 3
      });
    } else {
      ApplyDamageToTarget(ctx, e.uid, dmg);
    }
    totalDamage += dmg;
  }
  if (!isKojonn) {
...
export function Enemy_Heal_Self(ctx, enemyUID) {
  ...
  SpawnDamageText(ctx, heal, enemy.x ?? 0, enemy.y ?? 0, 'heal');
  ...
  setTimeout(() => {
    ...
    if (!g.RoundActive) {
      ...
      if (!arr.length) {
        ...
        const idx = arr.findIndex(a => a.uid === currentUID);
        if (idx !== -1) {
          g.CurrentTurnIndex = idx;
        } else if (g.CurrentTurnIndex == null || g.CurrentTurnIndex >= arr.length) {
          g.CurrentTurnIndex = 0;
        }
        const curType = GetCurrentType(ctx);
        g.TurnPhase = curType === 0 ? 0 : 2;
        g.CanPickGems = curType === 0 ? 1 : 0;
      }
      g.IsPlayerBusy = 0;
    }
  }, respawnDelay * 1000);
...
export function ResolveGemAction(ctx, gemColor, actorUID) {
  ...
  if (gemColor === 3) {
    ...
    TryGrantSpeedExtraTurn(ctx, actorUID);
    AdvanceTurn(ctx);
    ProcessTurn(ctx);
    return;
  }
...
export function BuildRoundGroups(ctx) {
  ...
  const jitter = g.RoundJitter ?? 0.5;
  ...
  g.TurnOrderArray = withInit.map(a => ({ uid: a.uid, spd: a.spd, type: a.type }));
  g.CurrentTurnIndex = 0;
...
export function ExecuteSkill(ctx, skillId, actorUID) {
  ...
  if (skillId === 'DEF_UP') {
    ...
    g.ActionLockUntil = (g.time || 0) + 0.5;
...
export function ProcessTurn(ctx) {
  ...
  const order = g.TurnOrderArray || [];
  const orderLine = order.map((a, i) => {
  ...
  if (actor) {
    ...
    const buff = actor.kind === 'hero' ? (g.PartyBuff_SPD || 0) : 0;
    console.log(`[TURN] idx=${g.CurrentTurnIndex} ${actor.name || uid} type=${type} spd=${Math.round(base + buff)}/${Math.round(base)} (+${Math.round(buff)})`);
  }
...
export function SpawnDamageText(ctx, amount, x, y, kind = 'damage') {
  const g = getGlobals(ctx);
