Original prompt: Expose deterministic test hooks so Playwright can validate turns, skills, and board resolution without manual play (dev-only, no gameplay changes, canvas game).

Notes
- Add window.render_game_to_text with concise state snapshot for Playwright.
...
- Run Playwright client if requested.
...
- drawFrame now accepts optional dt override.
 ...
 - Playwright test burst (KeyA devtest) confirms turn order advances across rounds (see /tmp/web-game-turns-2).
